...
const TriggerNode: React.FC<NodeProps> = ({ data, selected, id }) => {
  ...
  return (
    <div className={`relative group ${selected ? 'ring-2 ring-blue-500 ring-offset-2' : ''} ${nodeData?.isStartNode ? 'ring-2 ring-green-500 ring-offset-2' : ''}`}>
      <div className="bg-purple-500 text-white px-4 py-3 rounded-lg shadow-lg min-w-[180px] max-w-[220px]">
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-semibold text-sm">{nodeData?.label || 'Trigger'}</span>
          </div>
        ...
        {/* Status information */}
        {nodeData?.triggerSelectType === 'node' && nodeData?.selectNode && (
          <div className="text-xs opacity-75 bg-purple-600 px-2 py-1 rounded">
            {nodeData.selectNode} - {nodeData.action || nodeData.triggerType}
          </div>
        )}
        {nodeData?.triggerSelectType !== 'node' && nodeData?.table && (
          <div className="text-xs opacity-75 bg-purple-600 px-2 py-1 rounded">
            {nodeData.table} - {nodeData.action || nodeData.triggerType}
          </div>
        )}
      </div>
