...
const { processAIWithModel } = require('../llmservice');

class CrmApprovalNodeExecutor {
  constructor(io) {
    this.io = io;
  }
  ...
  async execute(nodeInstance, node, workflowInstance, data = {}) {
    try {
      ...
      // If CRM visualization is not present yet, try to generate it from previous Prompt node
      if (!workflowData.graph_data?.visualization?.nodes?.length && !workflowData.crmGraphGenerated) {
        try {
          // Get completed nodes and find last promptNode
          const completedNodes = await WorkflowNodeInstance.getCompletedNodes(workflowInstance.id);
          const promptNodeInstance = [...completedNodes].reverse().find(n => n.node_type === 'promptNode');

          if (promptNodeInstance && promptNodeInstance.result) {
            let promptResult = promptNodeInstance.result;
            ...
            const rawContent = typeof promptResult === 'string'
              ? promptResult
              : (promptResult.content || promptResult.result || JSON.stringify(promptResult));

            if (rawContent) {
              const systemPrompt = `You are a CRM data transformer.
You receive arbitrary text or semi-structured content and must convert it into STRICT JSON for CRM visualization.
      ...
      "nodes": [
        {
          "id": "string",
          "label": "string",
          "group": "company" | "person" | "opportunity",
          "color": "string (hex)",
...
Rules:
- Use group "company" for accounts, "person" for people/contacts, "opportunity" for deals/opportunities.
- IDs must be unique strings.
- If you are unsure, make best-effort guesses but keep JSON valid.
- Do NOT wrap the JSON in markdown or any extra text.`;

              const aiResult = await processAIWithModel(
                systemPrompt,
                typeof rawContent === 'string' ? rawContent : JSON.stringify(rawContent),
                4000,
                nodeConfig.api || 'openai',
                nodeConfig.model || 'gpt-4',
                'txt'
              );

              if (aiResult && typeof aiResult !== 'string' && aiResult.result) {
                let parsed;
                try {
                  parsed = typeof aiResult.result === 'string'
                    ? JSON.parse(aiResult.result.trim())
                    : aiResult.result;
                } catch (e) {
                  console.error('Failed to parse CRM graph_data JSON from AI:', e);
                }

                if (parsed?.graph_data?.visualization?.nodes?.length) {
                  workflowData.graph_data = parsed.graph_data;
                  workflowData.crmGraphGenerated = true;
                  await WorkflowInstance.updateData(workflowInstance.id, workflowData);
                  workflowInstance.data = workflowData;

                  await WorkflowNodeInstance.logExecution(
                    workflowInstance.id,
                    nodeInstance.id,
                    'info',
                    'CRM graph_data generated from Prompt node output',
                    { sourceNodeInstanceId: promptNodeInstance.id }
                  );
                }
              }
            }
          ...
          console.warn('CRM Approval: failed to generate graph_data from Prompt node:', transformError);
        }
      }
      ...
      if (approver.id && !String(approver.id).startsWith('email_')) {
        ...
        const approvalQuery = `
          ...
          VALUES ($1, $2, $3, 'pending', 'crmApprovalNode')
          RETURNING *
        `;
        ...
        approvalRecord = {
          ...
          status: 'pending',
          node_type: 'crmApprovalNode'
        };
      }

      // Send socket event to frontend for approval
      const approvalEvent = {
        type: 'workflow_crm_approval_required',
      ...
      if (approver.id) {
        ...
        const redirectUrl = `/crm-approval?id=${nodeInstance.id}`;
  ...
  async handleApprovalDecision(workflowInstanceId, nodeInstanceId, approvalId, decision, comments, approverId, approverName, selectedCrmItems = [], assignedSellers = {}) {
    try {
      // Log form submission
      await WorkflowNodeInstance.logExecution(
        workflowInstanceId,
      ...
      const result = await WorkflowNodeInstance.updateStatus(nodeInstanceId, 'completed', { decision, comments, approverId }, {decision, approverId, approverName});
      console.log("---result 162---",result);
      ...
      const workflow_data = workflowInstance.data;
      workflow_data.selectedCrmItems = selectedCrmItems;
      workflow_data.assignedSellers = assignedSellers;

      await WorkflowInstance.updateData(workflowInstanceId, workflow_data);
...
module.exports = CrmApprovalNodeExecutor; 