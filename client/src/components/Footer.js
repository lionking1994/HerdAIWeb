...
import { FloatingAgent } from './MeetingAgent/FloatingAgent';
export default function Footer({ bodyRef }) {
    ...
    const handleContactSubmit = async (e) => {
        ...
        const contactData = {
            username: e.target['contact-name'].value,
            useremail: e.target['contact-email'].value,
            usermessage: e.target['contact-message'].value,
        };

        try {
            // Send the Axios request
            await axios.post(`${process.env.REACT_APP_API_URL}/contactus/send`, contactData,
                {
                    headers: {
    ...
    const handleFileUpload = (file) => {
        ...
        // You can also trigger the upload here if you want to upload immediately
    };
    ...
    const handleFeedbackSubmit = async (e) => {
        ...
        formData.append('subject', e.target['feedback-subject'].value);
        formData.append('details', e.target['feedback-message'].value);
        ...
        try {
            await axios.post(
                `${process.env.REACT_APP_API_URL}/feedback/save-feedback`,
                formData,
                {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`,
    ...
    return (
        <>
            <FloatingAgent />
            <footer className="sticky bg-gray-800 p-1 text-white flex flex-col items-center w-full pb-12 md:pb-0 z-[100]">
                ...
                {isFeedbackModalOpen && (
                    <div className="fixed inset-0 flex items-center justify-center bg-[#0008] z-[100]">
