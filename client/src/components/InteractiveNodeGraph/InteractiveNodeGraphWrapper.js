...
const createLayout = (nodes, edges) => {
  const nodeWidth = 172;
  const nodeHeight = 60;
  const spacing = 120;
    ...
    for (const node of queue) {
      ...
      for (const neighbor of adjacencyList[node.id] || []) {
        ...
        if (inDegree[neighbor] === 0) {
          nextQueue.push(nodes.find(n => n.id === neighbor));
        }
      }
    }
...
const InteractiveNodeGraphInner = ({
  ...
  const [nodeAnnotations, setNodeAnnotations] = React.useState({});
  const [edgeAnnotations, setEdgeAnnotations] = React.useState({});

  const handleAnnotationClick = React.useCallback((id, type = 'node') => {
  ...
  const exportGraph = React.useCallback(async () => {
    try {
      ...
      <div style={{ 
        ...
        <div style={{
          ...
          <select
            value={exportFormat}
            onChange={(e) => {
              // You can add state management for export format if needed
            }}
            style={{
              padding: '4px 8px',
...
const InteractiveNodeGraph = (props) => {
  return <InteractiveNodeGraphInner {...props} />;
};
