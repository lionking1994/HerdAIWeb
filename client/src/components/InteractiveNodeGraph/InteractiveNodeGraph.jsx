...
const InteractiveNodeGraphInner = ({
  jsonData,
  meetingId,
  templateId,
  tPrompt,
  ...
  const [edgeAnnotations, setEdgeAnnotations] = useState({});
  ...
  const handleAnnotationClick = useCallback((id, type = 'node') => {
  ...
  const convertToReactFlowFormat = useCallback(() => {
    ...
    const nodes = jsonData.visualization.nodes.map(node => ({
      id: node.id,
      type: 'custom',
      position: { x: 0, y: 0 },
      data: {
        ...
        onAnnotationClick: (nodeId) => handleAnnotationClick(nodeId, 'node')
      }
    }));

    const edges = jsonData.visualization.edges.map((edge, index) => {
      const edgeId = `${edge.from}-${edge.to}-${index}`;

      // Use custom edge type now that handles are fixed
      const edgeType = 'custom';
      const strokeWidth = Math.max(1, (edge.thickness || 1) * 2);
      ...
      return {
        ...
        data: {
          ...
          onAnnotationClick: (edgeId) => handleAnnotationClick(edgeId, 'edge')
        }
      };
    });
  ...
  const getNodeIconAndColor = (group) => {
    switch (group) {
      case 'person':
      case 'people':
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <span style={{ fontSize: '15px', color: '#374151', fontWeight: '500' }}>People</span>
              </div>
              <span style={{
                fontSize: '18px',
                fontWeight: '700',
...
const InteractiveNodeGraph = (props) => (
  <ReactFlowProvider>
    <InteractiveNodeGraphInner {...props} />
  </ReactFlowProvider>
);
